...
const CreateResourceItemType = ({ displayName, icon: Icon, color }: CreateResourceItemTypeProps) => {
    ...
    return (
        <Button variant="ghost" style={{ '--resource-color': color } as CSSProperties} className={`flex ${isCollapsed ? 'rounded-full' : ''} gap-1 text-amber-100 justify-between cursor-pointer h-auto p-2 bg-(--resource-color) hover:bg-[color-mix(in_srgb,var(--resource-color),black_20%)]`}>
