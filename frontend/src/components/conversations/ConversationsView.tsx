...
type ConversationViewProps = {
    sortMethod: (a: Conversation, b: Conversation) => number;
    searchValue: null | string;
}

const ConversationsView = ({ sortMethod, searchValue }: ConversationViewProps) => {
    ...
    const filteredConversations = searchValue
        ? sortedConversations.filter((conversation) => conversation.title.toLowerCase().includes(searchValue.toLowerCase()))
        : sortedConversations
    ...
    return (
        <div className="grid w-full grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5">
            ...
            {!isLoading ? filteredConversations?.map((conversation) => <ConversationCard key={conversation.id} conversation={conversation} />) : Array.from({ length: 4 }).map((_, i) => <ConversationCardSkeleton key={i} />)}
            {error && (
                <Card className="h-[240px] w-full">
