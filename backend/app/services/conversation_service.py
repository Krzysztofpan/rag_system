...
class ConversationMetadata(BaseModel):
    title: str = Field(description="Conversation title. General and short.")
...
class ConversationService:
    ...
    async def update_from_summary(
        ...
        *,
        user_id: UUID,
    ) -> tuple[str, ConversationTopicName]:
        prompt = ChatPromptTemplate.from_template(CONVERSATION_METADATA_TEMPLATE)
        conversation = await self.get_conversation(conversation_id, user_id=user_id)
        metadata_llm = ChatOpenAI(model="gpt-4o-mini").with_structured_output(
            ConversationMetadata
        )
        metadata_chain = prompt | metadata_llm
        result = await metadata_chain.ainvoke(
